...
document.addEventListener('DOMContentLoaded', function() {
    const selectedCottage = document.getElementById('cottage-select').value || ''; // Default to fetching all if none selected
    fetchReservations(selectedCottage);
});

// Function to fetch reservations for selected cottage or all cottages
function fetchReservations(selectroom) {
    const queryParam = selectroom ? `?selectroom=${selectroom}` : '';

    fetch(`AdminRoomfunction.php${queryParam}`)
...
function openAddForm(selectroom) {
    // Set the cottage number in the hidden field
    document.getElementById('selectroom').value = selectroom;
...
function openEditForm(reservationId) {
    ...
    const [lastname, firstname, middlename, contactnumber, email, address, note, checkindate, checkoutdate, checkin, checkout, guests, selectroom, reference] = reservationData;
    ...
    document.getElementById('editSelectCottage').value = selectroom;
...
document.getElementById('editReservationForm').addEventListener('submit', function(event) {
    ...
    const selectroom = document.getElementById('editSelectCottage').value;
    ...
    const data = `id=${reservationId}&firstname=${firstname}&lastname=${lastname}&middlename=${middlename}&contactnumber=${contact}&email=${email}&address=${address}&note=${note}&checkindate=${checkindate}&checkoutdate=${checkoutdate}&selectroom=${selectroom}&checkin=${checkin}&checkout=${checkout}&guests=${guests}&reference=${reference}`;

    // Create an AJAX request
    const xhr = new XMLHttpRequest();
    xhr.open('PUT', 'AdminRoomfunction.php', true);
        ...
        if (xhr.status === 200) {
            try {
                ...
                if (response.success) {
                    ...
                    updateReservationRow(reservationId, lastname, firstname, middlename, contact, email, address, note, checkindate, checkoutdate, checkin, checkout, selectroom, guests);
                    closeEditModal();
                } else {
...
function updateReservationRow(reservationId, lastname, firstname, middlename, contact, email, address, note, checkindate, checkoutdate, checkin, checkout, selectroom, guests, reference) {
    const reservationRow = document.querySelector(`tr[data-id='${reservationId}']`);
    ...
    reservationRow.cells[12].innerText = selectroom;
